...
@click.pass_context
def cli(ctx):
    """Flouri - AI-powered terminal environment (flouri.sh).
...
def agent_command(prompt: str, allowlist: str | None, blacklist: str | None, stream: bool):
    ...
    Example:
        flouri agent "What is the difference between git merge and rebase?"
        flouri agent "List all files in the current directory"
        flouri agent -a "ls,cd,git" "Check git status"
        flouri agent -b "rm,dd" "Help me organize files"
        flouri agent --stream "Explain Docker containers"  # Live streaming output
    """
    try:
